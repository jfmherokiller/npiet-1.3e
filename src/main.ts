...
let do_gdtrace = 0;
let gd_trace_filename = "npiet-trace.png";
let gd_trace_simple = 0;
let gd_trace_start = 0;
let gd_trace_end = 1 << 31;		/* lot's to prlet*/
...
function printf(args,args2?:any)
{
...
function vprintf(args,args2?:any)
{
    if (verbose)
        printf(args)
}
...
extern void alloc_cells (letn_width, letn_height);
 ...
 */
let n_hue	=	6	;	/* 4 colors */
let n_light	=	3	;	/* 4 shades */
let c_white	=	(n_hue * n_light);
let c_black	=	(c_white + 1);
let n_colors=	(c_black + 1);
/* internal used index for filling areas: */
let c_mark_index	=9999;

function adv_col(c, h, l)
{
    let columsMod6 = c % 6;
    let columsDivide6 = c/6;
    let hightPlusMod6 = columsMod6 + h;
...
let c_color ={
    letcol:'',			/* rgb color */
    l_name:'',			/* long color name */
    s_name:'',			/* short color name */
    letc_idx:'',			/* our internal color index */
};
    let c_colors= [
 ...
 * execution states:
 */
let p_dir_pointer;		/* DP: p_{left, right, up, down} */
let p_codel_chooser;		/* CC: p_left or p_right */
let p_xpos, p_ypos;		/* execution position */
...
let dp_dx(dp)	((dp) == 'l' ? -1 : ((dp) == 'r' ? 1 : 0))
let dp_dy(dp)	((dp) == 'u' ? -1 : ((dp) == 'd' ? 1 : 0))
 ...
 */
let stack = 0;		/* stack space */
let num_stack = 0;		/* current number of values on stack */
let max_stack = 0;		/* max size of stack allocated */
...
function alloc_cells (letn_width, letn_height)
{
    let i, j;
    let malloc;
    let n_cells = malloc (n_width * n_height * sizeof(int));
...
function  gd_save()
function  gd_trace()
function  gd_try_init()
function  gd_try_step(a1,a2,a3,a4,a5,a6)
function  gd_action(a1,a2,a3,a4,a5,a6,a7)
...
let i_sign(x)    ((x) < 0 ? -1 : ((x) > 0 ? 1 : 0))
let i_abs(x)     ((x) < 0 ? -(x) : (x))
let i_max(x, y)  ((x) > (y) ? (x) : (y))
...
void
    gd_arrow_pp (letx1, lety1, letdp, letgd_col)
{
    leti;
...
void
    gd_arrow (letx1, lety1, letx2, lety2, letdp, letgd_col)
{
#if 0
    leti;
...
gdPoletr_pts [4] = { { 0, 0 }, { 0, 1 }, { 0, 2 }, { 1, 0 } };
gdPoletl_pts [4] = { { 0, 0 }, { 0, 1 }, { 0, 2 }, { 1, 2 } };
gdPoletd_pts [6] = { { 0, 2 }, { 0, 3 }, { 1, 0 }, { 1, 1 },
{ 1, 2 }, { 1, 3 } };
gdPoletu_pts [7] = { { 0, 0 }, { 0, 1 }, { 0, 2 },
{ 1, 2 }, { 2, 0 }, { 2, 1 }, { 2, 2 } };
...
let gd_ch_pts(c)  (c == 'r' ? r_pts : (c == 'l' ? l_pts : \
(c == 'd' ? d_pts : (c == 'u' ? u_pts : \
(c >= 0 && c <= 9 ? num_pts [c] : 0)))))
let gd_ch_plen(c) (c == 'r' ? 4 : (c == 'l' ? 4 : (c == 'd' ? 6 : \
c == 'u' ? 7 : (c >= 0 && c <= 9 ? num_pts_plen[c] : 0))))
let gd_ch_pw(c)   (c == 0 || c == 4 || c == 6 || c > 7 ? 3 : \
(c == 1 ? 1 : 2))


void
    gd_paint_ch (letx1, lety1, letch, letgd_col)
{
    gdPolet*pts;
    leti, n_pts;
...
void
    gd_paint_num (letx1, lety1, letnum, letgd_col)
{
    gdPolet*pts;
...
void
    gd_alloc_piet_colors (gdImagePtr im, let*col)
{
    leti;
 ...
 */
void
    gd_try_step_pp (lettry, letn_x, letn_y, letdp, letcc)
{
    letx1 = n_x * c_xy;
    lety1 = n_y * c_xy;
    letx2, y2, x3, y3;
 ...
 */
void
    gd_try_step (letexec_step, lettries, letn_x, letn_y,
    letdp, letcc)
{
    char tmp [128];
...
void
    gd_action (letp_x, letp_y, letn_x, letn_y, leta_x, leta_y, char *msg)
{
...
static void PrintGifError(leterrorCode)
{
 ...
 */
void
    c_check (leti, letc, let*last_c, let*last_p, let*min_w)
{
    if (i == 0) {
    *last_c = c;
    *last_p = i;
    ...
    cleanup_input ()
{
    ...
    /* make a copy: */
    o_cells = (let*) malloc (width * height * sizeof(int));
 ...
 */
int
check_connected_cell (letx, lety, letc_idx, letc_mark,
let*n_x, let*n_y, let*num_cells)
{
    letc, found;
        ...
        if (a_col == c_white) {
            ...
            if (a_col >= 0 && a_col != c_black) {
                ...
                if (version_11) {
                     ...
                     */
                    let*visited = NULL, visited_len = 0, i;
                    white_crossed = 1;
